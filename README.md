...
&emsp; 1) <code> PUSH < RAM | REG | NUM > </code> -- add argument with type specified in parentheses
&emsp; 2) <code> IN </code>  -- read from input the number  and add it to stack 
&emsp; 3) <code> POP < RAM | REG > </code> -- retrieves an element from the stack adds it to register or RAM
&emsp; 4) <code> ADD </code> -- pops 2 numbers from stack and add their sum to stack 
&emsp; 6) <code> SUB </code> -- pops 2 numbers from stack and add their difference to stack
&emsp; 7) <code> MUL </code> -- pops 2 numbers from stack and add their product to stack
&emsp; 8) <code> DIV </code> -- pops 2 numbers from stack and add their quotient to stack
&emsp; 9) <code> SQRT </code> -- pop number from stack and add square root of it
&emsp; 10) <code> SIN </code> -- pop number from stack and add sin(number)
&emsp; 11) <code> COS </code> -- pop number from stack and add cos(number)
&emsp; 12) <code> OUT </code> -- pop number from stack and print it
&emsp; 13) <code> HLT </code> -- exit from program
&emsp; 14) <code> JMP < LBL > </code> -- go to line 
&emsp; 15) <code> JA < LBL > </code>  -- pop 2 numbers from stack, if first "<" second, then go to line
&emsp; 16) <code> JAE < LBL > </code> -- pop 2 numbers from stack, if first "<=" second, then go to line
&emsp; 17) <code> JB < LBL > </code>  -- pop 2 numbers from stack, if first "> second, then go to line
&emsp; 18) <code> JBE < LBL > </code> -- pop 2 numbers from stack, if first ">=" second, then go to line
&emsp; 19) <code> JE < LBL > </code>  -- pop 2 numbers from stack, if first "==" second, then go to line
&emsp; 20) <code> JNE < LBL > </code> -- pop 2 numbers from stack, if first "!=" second, then go to line
&emsp; 22) <code> CALL < LBL > </code> -- go to label, and add the last line to call stack
&emsp; 23) <code> RET </code> -- pop label from stack and go to it
&emsp; 24) <code> PUTC </code> -- pop number from stack and print it such as symbol
&emsp; 25) <code> SET_PIXEL </code> -- dyes pixel(rbx, rax) color = RAM[rcx] 
&emsp; 26) <code> CREATE_WINDOW </code> -- create window 256 by 256
<br>
